...
import com.packt.masteringakka.bookstore.order._
...
class SalesOrderProcessorUnitSpec extends FlatSpec with Matchers with BeforeAndAfterAll with MockitoSugar{
  import com.packt.masteringakka.bookstore.order.SalesOrderProcessor._
  implicit val system = ActorSystem()

  class scoping extends TestKit(system) with ImplicitSender{
    val userMgr = TestProbe()
    val bookMgr = TestProbe()
    val creditHandler = TestProbe()
    val namesMap =
      Map(
